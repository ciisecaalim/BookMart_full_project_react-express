...
// Create Admin (allow initial registration if no admin exists)
const createAdmin = async (req, res) => {
  try {
    ...
    if (count > 0) {
      // Only allow logged-in admin to create new admin
      if (!req.admin || req.admin.role !== "admin") {
        return res.status(401).json({ error: "Only admin can create new admin" });
      }
    }
...
// Ensure default admin if no admin exists
