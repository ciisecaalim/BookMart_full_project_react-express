...
function OrdersPage() {
  ...
  const filteredOrders = orders
    ...
    .filter((o) => {
      ...
      return (
        ...
        o.products.some((p) => p.productId?.name?.toLowerCase().includes(key))
      );
    });
  ...
  const exportCSV = () => {
    ...
    const rows = dataToExport.map(o => [
      ...
      o.products.map(p => `${p.productId?.name || "Unknown"} (x${p.quantity})`).join("; "),
          ...
          <tbody>
            ${dataToPrint.map((o, i) => `
              <tr>
                ...
                <td>${o.products.map(p => `${p.productId?.name || "Unknown"} (x${p.quantity})`).join("; ")}</td>
          ...
          <table className="min-w-full">
            ...
            <tbody>
              {currentOrders.map((order, index) => (
                <>
                  <tr key={order._id} className={`${index%2===0?'bg-gray-50':'bg-white'} ${order.status==='Pending'?'bg-yellow-50':''}`}>
                        ...
                        <div className="space-y-3">
                          <div className="bg-white p-3 rounded-xl shadow-sm border">
                            ...
                            {order.products.map(p=>(
                              <div key={p.productId?._id} className="flex justify-between text-sm mb-1">
